...
- **range 192.168.2.144 192.168.254;** - диапазон выдаваемых адресов (их тут 110, как требуется по заданию, делаем также как я, просто меняем третий октет)
- **option domain-name-servers 172.16.2.2, 8.8.8.8, 8.8.8.4;** - IP-адрес будущего туннельного интерфейса DNS-сервера (SRV2), и IP-адреса классических DNS-серверов (чтобы у SRV2 и CLI1 был нормальный выход в Интернет). Пока мы туннель и DNS не настроили, но все равно пишем как я.
- **option routers 192.168.2.1;** - шлюз, который будет выдаваться клиентам. Это адрес интерфейса SRV1 с LAN-сегментом "lan".
...
Проверяем доступ в Интернет классическим образом - ping 8.8.8.8 (на SRV1 и CLI1).
...
В конце файла **/var/www/wordpress/wp-config.php** надо еще добавить две строчки с указанием https ссылки на IP-адрес (опять NAT). Кстати, рекомендую при настройке Wordpress использовать только один IP-адрес, и именно тот, где интерфейс NAT, чтобы вы могли потом сразу и легко получить доступ к админской панели прямо с хостового браузера.
